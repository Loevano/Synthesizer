...
void SynthController::submitRealtimeCommandOrApply(RealtimeCommand command) {
    ...
    switch (command.type) {
        ...
        case RealtimeCommandType::MidiNoteOn:
            {
                ...
                heldMidiNotes_.erase(
                    std::remove(heldMidiNotes_.begin(), heldMidiNotes_.end(), command.noteNumber),
                    heldMidiNotes_.end());
                heldMidiNotes_.push_back(command.noteNumber);
                activeMidiNote_.store(command.noteNumber, std::memory_order_release);
            }
            break;
        ...
        case RealtimeCommandType::MidiNoteOff:
            {
                std::lock_guard<std::mutex> noteLock(midiNoteStateMutex_);
                const auto newEnd = std::remove(heldMidiNotes_.begin(), heldMidiNotes_.end(), command.noteNumber);
                if (newEnd != heldMidiNotes_.end()) {
                    heldMidiNotes_.erase(newEnd, heldMidiNotes_.end());
                    activeMidiNote_.store(
                        heldMidiNotes_.empty() ? -1 : heldMidiNotes_.back(),
...
void SynthController::handleNoteOnLocked(int noteNumber, float velocity) {
    {
        ...
        activeMidiNote_.store(noteNumber, std::memory_order_release);
    }
...
void SynthController::handleNoteOffLocked(int noteNumber) {
    {
        ...
        heldMidiNotes_.erase(newEnd, heldMidiNotes_.end());
        activeMidiNote_.store(heldMidiNotes_.empty() ? -1 : heldMidiNotes_.back(), std::memory_order_release);
    }
